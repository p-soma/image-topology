...
fiberFile = fullfile(fiberPath,'e3.txt');

fiber_e3 = dlmread(fiberFile,',');
...
for s=S
    for p=P
        density_filtration = gaussianDensityFiltration(fiber_e3, s, p);
        filename = strcat('e3_s',num2str(s),'_p',num2str(p),'.txt');
