function [rawPatches, patches] = createPatchSample(n, cut, dim, imgPath)
...
%     GMEAN = 932.6349;
    ...
    rawPatches = zeros(total_patches,dim);
    ...
    for image=images
        ...
        % remove a 2-pixel border around the left and right borders of image due to camera
        % limitations
        buf = buf(3:end-2,:);
        
        % select n random 9x9 patches from the image as 81-D vectors
        rawPatchSamp = getPatchVectors(buf, n, dim);
        
        % take the log intensity of each pixel
        patchSamp = log(1 + rawPatchSamp);
        ...
        keepRawPatches = rawPatchSamp(keepIDX,:);
        ...
        rawPatches(idx_start:idx_end,:) = keepRawPatches(1:patches_per_image,:);
